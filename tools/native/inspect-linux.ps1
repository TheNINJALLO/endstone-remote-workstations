...
try {
    ...
    $vcfOverride=New-VcfComposeContextOverride $vcfContext 'abi-lab'
    & $vcfDocker compose -f compose.yaml -f $vcfOverride --profile research build abi-lab
...
} finally {$env:VCF_LINUX_INPUTS=$vcfPreviousInputs;try {Remove-VcfBuildContext $vcfContext} finally {Pop-Location}}
