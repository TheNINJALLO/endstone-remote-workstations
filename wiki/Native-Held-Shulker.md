...
![Filled actual held shulker](https://raw.githubusercontent.com/TheNINJALLO/endstone-remote-workstations/v0.5.0-native.1/docs/images/native-held-storage/filled-shulker.png)

![Existing enchanted pickaxe transferred into the box](https://raw.githubusercontent.com/TheNINJALLO/endstone-remote-workstations/v0.5.0-native.1/docs/images/native-held-storage/enchanted-item.png)

The [release report](https://github.com/TheNINJALLO/endstone-remote-workstations/wiki/Native-Release) identifies packaged artifacts and limits. The screenshots above were captured with the exact release artifact. The [machine-readable record](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/research/native-release-0.5.0-native.1.json) binds command opening, quick-transfer round trip, forced cancellation and reopening to its installed hash. Clean-restart and initial stone checks also retain their earlier candidate evidence.
...
After restart, edits are quarantined for explicit `/vcf recovery` review. Use the displayed acceptance token only after checking actual inventory. The journal never automatically issues items. [Six BDS crash tests](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/docs/NATIVE_INVENTORY_CRASH_TESTS.md) establish detection and retained evidence, not automatic held-editor crash recovery.
