...
Examples are operator-only by default. Grant explicit permissions through your normal permissions plugin; the framework does not grant Creative/operator privileges. Start with `/vcf status`, `/vcf capabilities`, `/vcf sessions` and `/vcf diagnose`, then follow [the examples](https://github.com/TheNINJALLO/endstone-remote-workstations/wiki/Native-Examples).
...
Stop before replacing binaries. Preserve provider data and world backups together, verify checksums and test upgrades in a copy. [Recovery details](https://github.com/TheNINJALLO/endstone-remote-workstations/wiki/Native-Recovery) explain the unresolved BDS/plugin save boundary.
