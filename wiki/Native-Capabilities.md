...
The current Windows provider SHA-256 is `07b037e42a832571448022fdea6385b2db08e0839a9e5b611965aeac500e3dfb`. It and both SDK consumers passed [startup and clean shutdown](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/research/native-evidence/windows-171e0aa-startup-smoke.json), including loaded-file identities and the native primitive manifest. Stock-client UI tests for this exact Windows build remain outstanding. No catalog screen has completed gameplay qualification under the native Windows plugin.

The Linux Docker build passes locally and in CI and produces a real ELF `.so`. Exact local BDS/Endstone files are admitted for the public SDK. Stock-client forms, passive inventory observation, four shared inventory roles and eight opt-in original workstation contexts have [smoke evidence](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/docs/examples/linux-native-workstations.md). Three nearby linked machines also have [processing, delivery and cleanup smoke evidence](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/docs/examples/linux-linked-furnaces.md). Linked Ender Chest and barrel [transfers and cleanup](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/docs/examples/linux-linked-storage.md) also passed selected PC checks. Controlled ingredient-disconnect checks passed. Grindstone's initial output rejection remains unresolved despite successful repeats; the older missing-plank/death observation remains in its original record. Five [linked utilities](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/docs/examples/linux-linked-utilities.md) have selected gameplay evidence; the later `d6adf87` build passed repeated beacon/crafter SDK closure and selected restoration, with initial unaccepted inputs retained as limitations. The separate [linked hopper](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/docs/examples/linux-linked-hopper.md) passed selected slot transfers, metadata and SDK/native/source cleanup. Three [linked chest paths](https://github.com/TheNINJALLO/endstone-remote-workstations/blob/v0.5.0-native.1/docs/examples/linux-linked-chests.md) passed selected transfers, metadata and close/reopen checks, including denied-partner and removed-partner cleanup for an ordinary double chest. Other Linux catalog adapters remain incomplete. These results do not upgrade any catalog row to full qualification.

| Entry | Family | Windows original/custom | Linux original/custom |
|---|---|---|---|
| `chest` | storage | Not qualified / not implemented | Experimental nearby unpaired source, PC transfers/metadata and SDK/native close passed / not implemented |
| `doublechest` | storage | Not qualified / not implemented | Experimental nearby paired source, PC transfers/metadata, both-half guards and pair-loss cleanup passed / not implemented |
