...
int main(void){
 ...
 memset(&api,0xa5,sizeof(api));
 if(oni_vcf_get_api(VCF_ABI_VERSION_1_3,VCF_API_1_3_SIZE,&api)!=VCF_OK)return 14;
 if(api.size!=VCF_API_1_3_SIZE||api.version!=VCF_ABI_VERSION_1_3||!api.read_inventory_item)return 15;
 { const unsigned char* bytes=(const unsigned char*)&api;size_t i;
