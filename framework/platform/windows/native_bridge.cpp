...
Document education_state(endstone::Player& player) {
    Document result;
...
Document entity_state(endstone::Player& player,endstone::Actor& source,const std::string& kind) {
    const auto bound=entity_binding(player,source,kind);
    Document result; result["actor_id"]=bound.identity; return result;
}
struct RidingState {
...
Document entity_mount_state(endstone::Player& player,endstone::Actor& source) {
    const auto state=read_riding(player,source);
    if(!state.vehicle) return nullptr;
    Document result;
    result["vehicle_id"]=state.vehicle_id; result["vehicle_runtime_id"]=state.vehicle_runtime;
    return std::move(result);
}
Document entity_mount_link(endstone::Player& player,endstone::Actor& source) {
    const auto state=read_riding(player,source);
    if(!state.vehicle) return nullptr;
...
Document lectern_state(endstone::Player& player,int x,int y,int z) {
...
Document state(endstone::Player& player) {
    auto native=player_handle(player); Document result;
...
NativeState inspect(endstone::Player& player) {
 auto p=player_handle(player);return {ready(p),field<void*>(p,1440)!=nullptr,field<unsigned char>(p,3480)};
}
int station(endstone::Player& p,const std::string& kind,int x,int y,int z){return open_station(p,kind,x,y,z);}
int linked(endstone::Player& p,const std::string& kind,int x,int y,int z){return open_linked_station(p,kind,x,y,z);}
void refresh(endstone::Player& p){refresh_inventory(p);}
void shutdown_editors(){shutdown_sign_hooks();shutdown_command_hooks();shutdown_structure_hooks();}
}
