...
struct Bridge {
 ...
 struct State{uintptr_t player;bool ready;uint8_t window;};
 State inspect(endstone::Player& p)const{
  ...
  return {player,memory.field<uintptr_t>(player,1368)==0&&depth<=1,window};
 }
...
bool shared_inventory(std::string_view id){return id=="inventory2x2"||id=="armor"||id=="offhand"||id=="recipebook";}
...
struct NativeUi::Impl {
 struct View{vcf_handle owner,id;std::string player,kind,dimension,permission;uint32_t nonce;int window=-1;
  ...
  Clock::time_point queued=Clock::now(),next_check{};};
 ...
 void retire(View&v,vcf_status result){try{engine.retired(v.owner,v.id,result);}catch(const Error&) {}}
 bool poll(View&v){
  auto*p=player(v.player);const auto now=Clock::now();
  if(!p||!p->isValid()||v.closing||v.superseded){retire(v,VCF_CLOSED);return true;}
  if(!allowed(*p,v)){retire(v,VCF_DENIED);return true;}
  if(!v.prepared){require(bridge.inspect(*p).ready,VCF_CONFLICT);v.prepared=true;p->sendPacket(115,ping(v.nonce));return false;}
  if(!v.active){
   ...
   v.activating=true;
   uint8_t window;
   try{window=bridge.open(*p);}catch(...){v.activating=false;throw;}
   v.activating=false;
  ...
  if(v.close_seen||now>=v.next_check){
   ...
   if(bridge.inspect(*p).ready){retire(v,VCF_OK);return true;}
   engine.authorize(v.owner,v.id,VCF_GUARD_ACTIVE);
  }
  if(v.close_seen||now-v.queued>std::chrono::minutes(20)){retire(v,VCF_CLOSED);return true;}
  return false;
 }
};
NativeUi::NativeUi(endstone::Server&s,Engine&e):impl_(std::make_unique<Impl>(s,e)){}
NativeUi::~NativeUi()=default;
bool NativeUi::supports(std::string_view id){return shared_inventory(id);}
vcf_status NativeUi::open(const Session&s){
 if(!supports(s.kind)||s.mode!=VCF_REAL_SOURCE||!s.title.empty()||!s.rules.empty())return VCF_UNAVAILABLE;
...
vcf_status NativeUi::close(const Session&s){auto it=impl_->views.find(s.id);if(it!=impl_->views.end())it->second.closing=true;
...
void NativeUi::tick(){
 ...
 while(remaining--&&!impl_->views.empty()){
  ...
  catch(const Error&e){impl_->retire(it->second,e.status);done.push_back(it->first);}
  catch(...){impl_->retire(it->second,VCF_INTERNAL);done.push_back(it->first);}
...
void NativeUi::sent(endstone::PacketSendEvent&e){
 ...
 for(auto&[_,v]:impl_->views)if(!v.closing&&v.player==e.getPlayer()->getUniqueId().str()){
  try{
   if(e.getPacketId()==46){Reader r{e.getPayload()};auto window=r.byte(),type=r.byte();r.var();r.var();r.var();r.var(64);require(r.offset==r.data.size());
    if(v.activating&&!v.observed&&type==255&&window>=1&&window<=99){v.window=window;v.observed=true;}else v.superseded=true;
...
void NativeUi::shutdown(){impl_->views.clear();}
}
