...
class ReservedInventory:
    def __init__(self, state, read, write, *, locked=frozenset(), accept=lambda slot, item: True):
        self.state = self.baseline = state
        self.read, self.write = read, write
        self.history = OrderedDict()
        self.last_request = 0
        self.closed = False
        self.locked, self.accept = frozenset(locked), accept
    ...
    def apply(self, request):
        ...
        plan = plan_storage(self.state, request, locked=self.locked, accept=self.accept)
