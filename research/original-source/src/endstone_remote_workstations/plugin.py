...
class RemoteWorkstations(Plugin):
    ...
    def _open(self, sender, key):
        ...
        if row['id'] == 'npc':
            ...
            return
        if row['id'] in ('enderchest', 'shulker') and isinstance(sender, Player):
            try:
                client = self._ui.bind(self)
                opener = client.open_held_item if row['id'] == 'shulker' else client.open_ender_chest
                opener(sender, on_close=lambda info:
                    sender.send_error_message(info.reason or 'Inventory opening failed.')
                    if info.state == 'failed' and sender.is_valid else None)
            except Exception as error:
                sender.send_error_message(str(error))
            return
