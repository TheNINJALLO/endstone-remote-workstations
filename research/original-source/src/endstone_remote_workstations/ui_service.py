...
class UIService:
    ...
    def capabilities(self, client, player):
        ...
        rows = capabilities()
        for row in rows:
            if row['id'] == 'shulker' and self.packet is not None:
                reason = self.packet.reason(player, ender=False, held=True) if player is not None else 'Player admission is required.'
                row.update(available=reason is None, unavailable_reason=reason, backend='packet_held_shulker',
    ...
    def enqueue(self, client, player, target, on_open, on_close):
        ...
        elif isinstance(target, HeldItem):
            self._text(target.title, 128, 'held item title')
            kind = 'held:shulker'
    ...
    def _cancel_view(self, entry, player):
        ...
        elif isinstance(entry.target, (InventoryMenu, EnderChest, HeldItem)):
    ...
    def _poll(self, entry):
        ...
        if isinstance(entry.target, NpcDialogue):
            ...
            if entry.info.state == 'queued':
                ...
                if isinstance(entry.target, HeldItem):
                    entry.packet_session = self.packet.open(player, entry.target.title, held=True)
                else:
