...
class PacketBackend:
    ...
    def reason(self, player, *, ender=True, held=False):
        ...
        if held:
            if not self.settings.get('held_shulker', False):
                return 'Held shulker screens are disabled in config.toml.'
            if not all(hasattr(self.native.bridge, name) for name in
                       ('held_watch', 'held_epoch', 'held_release', 'held_apply', 'held_clone', 'held_pack', 'held_unpack')):
                return 'The native companion lacks the held inventory save guard.'
            if not player.has_permission('remoteworkstations.open.shulker'):
                return 'You do not have permission to open held shulkers.'
    ...
    def open(self, player, title='Ender Chest', icons=None, buttons=None, *, held=False):
        self.native._owner()
        reason = self.reason(player, ender=icons is None and not held, held=held)
        if reason:
        ...
        for name, guard in self.native.guards.items():
            if name != 'remote_workstations.developer_menus' and guard(player, 'shulker' if held else 'enderchest' if icons is None else 'inventory_menu') is not True:
        ...
        if held:
            from .held_storage import HeldBinding
            binding = HeldBinding(player, generation, self.allocators.get(identity, 1000000), self.registries[identity], self.native.bridge)
        else:
        ...
        engine = ReservedInventory(binding.initial, binding.read, binding.commit,
                                   locked=getattr(binding, 'locked', frozenset()),
                                   accept=getattr(binding, 'accept', lambda slot, item: True))
    ...
    def receive(self, event):
        ...
        if session is None or session.reason == 'superseded':
            return
        if getattr(session.binding, 'kind', None) == 'shulker':
            if event.packet_id == 144:
                from .held_input import inventory_action
                try:
                    mutates = inventory_action(payload)
                except CodecError:
                    mutates = True
                if mutates:
                    event.is_cancelled = True
                    session.reason = 'backing_item_action'
                return
            if event.packet_id in (30, 31, 32, 36, 48):
                event.is_cancelled = True
                slot = session.binding.source.slot
                # The client echoes equipment after receiving new backing NBT.
                # Ignore that echo while keeping the actual selected item fixed.
                if event.packet_id != 31 or len(payload) < 3 or payload[-3:] != bytes((slot, slot, 0)):
                    session.reason = 'backing_item_action'
                return
    ...
    def _tick(self, player, session):
        ...
        if not player.is_valid or player.is_dead or player.dimension.name != session.dimension:
            ...
            return
        held = getattr(session.binding, 'kind', None) == 'shulker'
        reason = self.reason(player, ender=session.binding.icons is None and not held, held=held)
        if reason:
        ...
        if session.stage == 0 and elapsed >= CLIENT_TRANSITION_SECONDS:
            block = self.server.create_block_data(getattr(session.binding, 'block_type', 'minecraft:chest'))
            ...
            tag = CompoundTag()
            tag.set('id', getattr(session.binding, 'actor_type', 'Chest'))
            ...
            self.send(player, 46, Open(session.window, getattr(session.binding, 'container_type', 0), *session.position).encode())
    ...
    def poll(self):
        ...
        for identity in tuple(self.sessions)[:16]:
            ...
            if player is None:
                ...
                if hasattr(session.binding, 'close'):
