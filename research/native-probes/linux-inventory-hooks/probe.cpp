...
class HookProbe:public endstone::Plugin {
 ...
 uintptr_t container(endstone::Player& p){
  require(p.isValid()&&!p.isDead()&&p.hasPermission("vcf_hook_probe.admin"),VCF_DENIED);
...
public:
 void onEnable()override{
  ...
  std::array<uintptr_t,3> entries{bridge->fn(0xbcfdea0),bridge->fn(0xade2e00),bridge->fn(0xb3cb4e0)};
  ...
  std::array<void*,3> trampolines{reinterpret_cast<void*>(entries[0]),reinterpret_cast<void*>(entries[1]),reinterpret_cast<void*>(entries[2])};
  std::array<void*,3> detours{reinterpret_cast<void*>(&saved),reinterpret_cast<void*>(&changed),reinterpret_cast<void*>(&set)};
  for(size_t i=0;i<3;++i)if(prepare(handle,&trampolines[i],detours[i])!=0)throw std::runtime_error(error(handle));
 ...
 void quit(endstone::PlayerQuitEvent& e){if(e.getPlayer().getUniqueId().str()==watched_id){state->watched.store(0,std::memory_order_release);report("quit");watched_id.clear();}}
 void onDisable()override{if(state){state->watched.store(0,std::memory_order_release);report("disabled");}}
 bool onCommand(endstone::CommandSender& sender,const endstone::Command&,const std::vector<std::string>& args)override{
  try{
   ...
   if(action=="unwatch"){state->watched.store(0,std::memory_order_release);watched_id.clear();report(action);sender.sendMessage("Private hook watch cleared.");return true;}
   auto players=getServer().getOnlinePlayers();require(players.size()==1);auto& p=*players.front();auto native=container(p);
   if(action=="watch"){require(state->watched.load()==0);watched_id=p.getUniqueId().str();state->watched.store(native,std::memory_order_release);report(action);sender.sendMessage("Private hook watch armed for one validated test inventory.");return true;}
...
ENDSTONE_PLUGIN("vcf_private_hook_probe","0.0.0",HookProbe){description="Private native inventory hook qualification";command("vcf_hook_probe").description("Isolated console inventory hook checks").usages("/vcf_hook_probe <action: string>").permissions("vcf_hook_probe.admin");permission("vcf_hook_probe.admin").default_(endstone::PermissionDefault::Operator);}
